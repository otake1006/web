...
<form action="topics-del.php" method="post">
    ...
    <label>対象:</label>
    <input type="text" name="subject" value="<?php echo $subject; ?>" readonly>　　　　　<?php print ("<td><a href='http://10.123.100.118/topics-res-ins.php?number=".$number."&title=".$title."&kinds=".$kinds."&subject=".$subject."' target='_blank'>コメント新規登録</a></td>");?>
...
try {
    ...
    $sql = 'SELECT * FROM topics_comment WHERE topics_id = :number';
	...
	while($result = $stmt->fetch(PDO::FETCH_ASSOC)){
		...
		print("<td><a href='http://10.123.100.118/topics-res-upd.php?cmt_id=".$result['comment_id']."&topics_id=".$result['topics_id']."&cmt_kinds=".$result['cmt_kinds']."&comment_text=".$result['comment_text']."&uaer_name=".$result['user_name']."' target='_blank'>編集</a></td>");
		print("<td><a href='http://10.123.100.118/topics-res-del.php?cmt_id=".$result['comment_id']."&topics_id=".$result['topics_id']."&cmt_kinds=".$result['cmt_kinds']."&comment_text=".$result['comment_text']."&uaer_name=".$result['user_name']."' target='_blank'>削除</a></td>");
        print("</tr>");
    }
	echo "</table>";
    // foreachの値を変数に格納したい
